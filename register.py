...
class Register:
    ...
    def reg(self):
        try:
            ...
            self.but = Button(self.window,text="Signup ", command=self.getting)
        ...
        try:
            ...
            if first_name == '' or last_name == '' or username == '' or password == '' or phone == '' or address == '':
                ...
                if tup != -1:
                    ...
                    qry = '''insert into user_reg (username,password,first_name,last_name,address,phone) values(%s,
                    %s,%s,%s,%s,%s) '''
                    values = (phone, password, first_name, last_name, address, username)
    ...
    def login(self):
        try:
            ...
            self.window1.configure(bg='light green')
            ...
            self.but1 = Button(self.window1, text="login", command=self.login_backend)
    ...
    def mainsc(self):
        try:
            self.window2=Tk()
            self.window2.title('Train Ticket Booking system')
            self.window2.geometry('600x400')
            self.lbl= Label(text="Choose Login Or Signup", bg="yellow", width="300", height="2", font=("Calibri", 13)).pack()
            self.lbl= Label(text="").pack()
            self.but = Button(self.window2, text="Signup", command=self.reg, bg='light blue', height="2",width="30" ,font="Bold 10")
            self.but.pack()
            self.but = Button(self.window2, text="LOGIN", command=self.login, bg='light blue', height="2",width="30", font="bold 10")
